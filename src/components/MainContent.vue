    ...
    <section>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="mb-4">
            <h3 class="text-md md:text-md font-bold">{{ article.title }}</h3>
            <p class="mt-2">{{ formatDate(article.created_at) }} • {{ article.readTime }} min read</p>
          </div>
          <p class="text-gray-700">{{ truncateContent(article.content, 50) }} <span class="text-blue-500 ">Lanjutkan baca</span></p>
        </div>
      </div>
    </section>
  </main>
